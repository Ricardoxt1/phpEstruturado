...
<head>
    ...
    <a href="../livros/cadastrar_livros.php">cadastrar</a>
    <a href="../livros/listar_livros.php">listar</a>
</head>
...
<body>
    ...
    <div>
        ...
        //primeira pagina
        echo "<a href='/projeto_biblioteca/livros/listar_livros.php?pagina=1'> Primeira </a>";

        //paginas subsequentes 
        for ($pag_ant = $pagina - 1; $pag_ant <= $pagina; $pag_ant++) {
            if ($pag_ant >= 1) {
                echo "<a href='/projeto_biblioteca/livros/listar_livros.php?pagina=$pag_ant'> $pag_ant </a>";
            }
        }

        for ($pag_seguinte = $pagina + 1; $pag_seguinte <= $pagina + $max_pg; $pag_seguinte++) {
            if ($pag_seguinte <= $quantidade_pg) {
                echo "<a href='/projeto_biblioteca/livros/listar_livros.php?pagina=$pag_seguinte'> $pag_seguinte </a>";
            }
        }

        //ultima pg
        echo "<a href='/projeto_biblioteca/livros/listar_livros.php?pagina=$quantidade_pg'> Ultima </a>";

        echo "<br>";
        echo "<h3> total de livros </h3>";


        for ($pagina_total = $pagina; $pagina_total <= $pagina; $pagina_total++) {
            $result_livros = "SELECT * FROM books LIMIT $pagina";
            $total_livros = mysqli_query($conn, $result_livros);
            while ($livros = mysqli_fetch_assoc($total_livros)) {
